...
function drawStateExecutionGraph(canvasId, xd) {
  ...
  var states = _.map(xd, function (state, stateName) {
    var r = _.merge({}, state);
    return _.merge(r, {stateName: stateName});
  });

  var stateNames = d3.keys(xd);
  var statNames = d3.keys(d3.values(xd)[0]).filter(function(key) { return key !== "totalActive"; });
  ...
  var state = svg.selectAll(".state")
    ...
    .attr("transform", function(d, k) {
      return "translate(" + x(d.stateName) + ",0)"; });
