 ...
 */
@Deprecated
public abstract class WorkflowDefinition<S extends Enum<S> & WorkflowState> extends AbstractWorkflowDefinition {
  ...
  protected WorkflowDefinition(String type, S initialState, S errorState, WorkflowSettings settings) {
    ...
    this.allStates = EnumSet.allOf((Class<S>) initialState.getClass()).stream().collect(toSet());
