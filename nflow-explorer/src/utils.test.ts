...
test('formatRelativeTime', () => {
  ...
  expect(formatRelativeTime('2021-05-26T06:25:40.722Z', now)).toBe(
    'less than a minute ago'
  );
  expect(formatRelativeTime(now, now)).toBe('less than a minute ago');

  expect(formatRelativeTime(new Date('2021-05-26T06:25:40.999Z'), now)).toBe(
    'in less than a minute'
  );
  expect(formatRelativeTime(new Date('2021-05-26T06:25:40.000Z'), now)).toBe(
    'less than a minute ago'
  );
