...
function CreateWorkflowInstanceForm(props: {
    ...
    const definitionFromType = (type: string | null) => props.definitions.filter(d => d.type === type)[0];

    const defaultDefinition = () => definitionFromType(queryParams.get('type')) || props.definitions[0];
    const [definition, setDefinition] = useState<WorkflowDefinition>(defaultDefinition());
    const [externalId, setExternalId] = useState<string>(queryParams.get('externalId') || '');
    const [businessKey, setBusinessKey] = useState<string>(queryParams.get('businessKey') || '');
    const [stateVariables, setStateVariables] = useState<string>(queryParams.get('stateVariables') || '');
    const [stateVariablesParsed, setStateVariablesParsed] = useState<{[key: string]: any}>();
