(function () {
  ...
  m.factory('Barchart', function () {
    ...
    function drawStateExecutionGraph(canvasSelector, statsData, definition, stateSelectedCallback) {
      ...
      var color = d3.scale.ordinal().range(['#98abc5', '#7b6888', '#a05d56', '#ff8c00']);
      ...
      function execTypeName(name) { return name === 'nonScheduled' ? 'Passive' : _.capitalize(name); }

      function removeFinalStates() {
        var copy = _.merge({}, statsData);
        var removeNames = [];

        _.forEach(copy, function(state, stateName) {
          var stateDef = _.first(_.filter(definition.states, function(e) { return e.id === stateName; }));
          if (stateDef && stateDef.type === 'end') { removeNames.push(stateName); }
        });

        _.forEach(removeNames, function(name) { delete copy[name]; });

        return copy;
      }

      function getStateNames(stats, definition) {
        var nonFinalStates = _.filter(definition.states, function(state) { return state.type !== 'end'; });
        var defStateNames = _.map(nonFinalStates, function(state) { return state.name; });
